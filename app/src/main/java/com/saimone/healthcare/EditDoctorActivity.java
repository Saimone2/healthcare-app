...
public class EditDoctorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        deleteButton.setOnClickListener(view -> {
            ...
            MyDialogFragment myDialogFragment = MyDialogFragment.newInstance("DELETE DOCTOR", fullname, address, experience, contactno, fee, specialty);
        ...
        updateButton.setOnClickListener(view -> {
            if (validateInput()) {
                ...
                try (Database db = new Database(getApplicationContext(), "healthcare", null, 1)) {
                    int result = db.updateDoctor(getIntent().getStringExtra("fullname"), getIntent().getStringExtra("hospital_address"), getIntent().getStringExtra("experience"), getIntent().getStringExtra("phone"), getIntent().getStringExtra("fee"), fullname, address, experience, contactno, fee, specialty);
                    if (result == 0) {
